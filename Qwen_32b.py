...
def _coerce_int(x) -> Optional[int]:
    try:
        ...
        if isinstance(x, float):
            # Only coerce floats that are extremely close to an int
            r = round(x)
...
def execute_python_code_return_int(code_text: str, timeout: int = 12) -> int:
    ...
    """
    # 1) Extract code inside ```python ... ```
    m = re.search(r"```python\s*\n(.*?)```", code_text, flags=re.DOTALL | re.IGNORECASE)
    if m:
        code = m.group(1).strip()
    else:
        # Heuristic fallback: gather lines that look like code
    ...
    if not code or len(code) < 4:
        return 0  # guarantee an int

    # 2) Run with timeout and restricted builtins
    ...
    try:
        ...
        # Try calling a solver function if present
        for fn_name in ("solve", "main"):
            ...
            if callable(fn):
                ...
                with redirect_stdout(out_buf2):
                    ret = fn()
                signal.alarm(0)
                # Re-check prints for sentinel or integer
                ...
                if v is not None:
                    return v
                # After calling, the function may have set `answer`
                for key in ("answer", "result", "final_answer", "ans", "solution", "output"):
                    if key in safe_locals:
                        v = _coerce_int(safe_locals[key])
                        if v is not None:
        ...
        # 3) Parse any integer from first execution's printed output
        if printed:
            m4 = re.findall(r"-?\d+", printed)
            if m4:
                return int(m4[-1])

        # 4) As a last parsing attempt, look in the code text itself
        ...
        # 5) Give up but return an integer
        return 0
...
def extract_final_answer(text: str) -> Optional[int]:
    """
    Robustly pull an integer (supports negatives) from CoT-style text.
    Priority on explicit Final Answer markers; fallback to last small integer in the tail.
    """
    patt = [
...
class QwenRunner:
    ...
    def generate_once(self, prompt: str) -> str:
        # Deterministic decoding to help accuracy
...
def code_only_prompt(problem: str) -> str:
    # STRICT template + sentinel to make extraction reliable.
